...
| `allowed_issue_statuses`  | An comma separated string. The detected jira issue's status will be compared against this list and if a match is not found then the status check will fail. Requires `validate_issue_status` to be set to `true`.                                                                                        | false    | In Progress |
